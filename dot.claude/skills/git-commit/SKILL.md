...
4. `git diff --cached` の内容に基づき、包括的なコミットメッセージを作成する
   ...
   - 1行目（`<type>[(optional scope)]: <description>`）は理由や背景を含めず、極限まで短い英文の動詞句で書く（例: `fix(claude): unbind ctrl+j to fix scroll conflict`。NG例: `unbind ctrl+j newline so scroll works while typing` のように理由まで詰め込む）。理由・背景は本文の箇条書きに書く
